...
# A1: Check if we're in the correct repository (modified for testing)
if [[ ! -f "$SCRIPT_DIR/run.sh" ]]; then
    error "run.sh not found in script directory: $SCRIPT_DIR"
    exit 1
fi
...
get_linear_issue() {
    local issue_id="$1"
    local query='{
        "query": "query($id: String!) { issue(id: $id) { id title description state { name } project { name } } }",
...
ISSUE_PROJECT=$(jq -r '.data.issue.project.name // "Unknown"' "$ISSUE_DATA_FILE")
...
log "Issue Project: $ISSUE_PROJECT"

# B3: Automatic project-based directory switching
log "Starting automatic project recognition and directory switching..."

# Read project dictionary from constitution.md
CONSTITUTION_FILE="$SCRIPT_DIR/commons/constitution.md"
WORKING_DIR=""

if [[ -f "$CONSTITUTION_FILE" ]] && [[ "$ISSUE_PROJECT" != "Unknown" ]]; then
    log "Project from Linear API: $ISSUE_PROJECT"

    # Parse project dictionary from constitution.md
    case "$ISSUE_PROJECT" in
        *"petit recipe"*|*"Petit Recipe"*)
            WORKING_DIR="$HOME/petit-recipe"
            PROJECT_NAME="recipe_app"
            ;;
        *"laminator"*|*"dashboard"*)
            WORKING_DIR="$HOME/laminator-dashboard"
            PROJECT_NAME="laminator_dashboard"
            ;;
        *"AI Assistant"*|*"Knowledge Hub"*)
            WORKING_DIR="$HOME/ai-assistant-knowledge-hub"
            PROJECT_NAME="knowledge_hub_mng"
            ;;
        *)
            warn "Unknown project: $ISSUE_PROJECT"
            PROJECT_NAME="knowledge_hub_mng"  # Default fallback
            ;;
    esac

    # Attempt automatic directory switching
    if [[ -n "$WORKING_DIR" ]] && [[ -d "$WORKING_DIR" ]]; then
        log "Switching to project directory: $WORKING_DIR"
        cd "$WORKING_DIR" || warn "Failed to switch to $WORKING_DIR"
        success "Successfully switched to: $(pwd)"
    elif [[ -n "$WORKING_DIR" ]]; then
        warn "Project directory not found: $WORKING_DIR"
        warn "Continuing in current directory: $(pwd)"
        PROJECT_NAME="knowledge_hub_mng"  # Fallback
    fi
else
    warn "Project dictionary not found or no project information available"
    # Fallback to legacy project detection
    PROJECT_NAME=""
...
log "Final project context: $PROJECT_NAME"
